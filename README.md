...
- **文件名翻译**：创建包含中文的文件时，自动翻译为英文文件名（支持路径翻译）
...
当创建包含中文的文件时（如`案例/测试.js`），扩展会自动检测并弹出翻译建议，选择后将文件重命名为英文（如`cases/test.js`）。

### 功能说明

- 自动检测中文文件名，弹出快速选择框
- 支持多种命名格式：camelCase、PascalCase、snake_case、param-case、pathCase
- 支持路径翻译：`你好/世界/测试.js` → `hello/world/test.js`
- 自动删除空的中文目录
- 可通过配置启用/禁用
...
```json
{
  "varTranslation.autoTranslateFileName": true
}
```
