...
- **文件名翻译**：在资源管理器中右键文件/目录，一键翻译文件名并转换为驼峰命名格式（支持 `Cmd+Z` 撤销）
- **快捷键**：`Cmd + Shift + T` — 编辑器选中文本时翻译文本，资源管理器选中文件时翻译文件名

## ⌨️ 快捷键

| 平台 | 快捷键 | 说明 |
|------|--------|------|
| **macOS** | `Cmd + Shift + T` | 编辑器：翻译选中文本；资源管理器：翻译文件名 |
| **Windows / Linux** | `Alt + Shift + T` | 同上 |

## 📁 文件名翻译

在**资源管理器**中右键文件或目录，选择「驼峰命名转换」或「驼峰翻译」子菜单中的具体格式：

- 中文文件名 → 自动翻译为英文 → 应用命名格式 → 重命名
- 英文文件名 → 直接应用命名格式 → 重命名（也可翻译成中文）
- 支持 `.eslintrc`、`.npmignore` 等 dotfile
- 支持目录重命名
- 支持 `Cmd+Z` 撤销重命名

![文件命名转换](images/vscode1.gif)
